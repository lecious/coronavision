...
#if defined(NDEBUG)
# error "Coronavision cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x515d82a6ad91b4ed7f16c766d20fac5ce8e55aaba1a15cf0341b203d044fa468");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coronavision: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coronavision Signed Message:\n";
...
{
    // Coronavision: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Coronavision: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Coronavision: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Coronavision: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coronavision: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coronavision: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfda05048c52abf76d74d0b88119e6bd6caded7cd91faba788de811da04a6c27a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "15/05/2020 Coronavision eys";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048238a9bc27905dd3d469bea4c07d72041d7206ca8ab27d8cb00f8801a94d8182d1c59466ba7c3ecfee11ceb310a782e7dcb698c7fd79fd7e22b13c38c089ab9f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1589498809;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084607507;

        if (fTestNet)
        {
            block.nTime    = 1589498809;
            block.nNonce   = 385840362;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd79a188ddc9a22aaf3524a5e9bf95639b2d257ada10b340d23e10d905f6640f9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb7, 0xdd }; // Coronavision: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoronavisionMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoronavisionMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoronavisionMiner(CWallet *pwallet)
{
    printf("CoronavisionMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coronavision-miner");
    ...
    try { loop {
        ...
        printf("Running CoronavisionMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoronavisionMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoronavisionMiner, pwallet));
}
